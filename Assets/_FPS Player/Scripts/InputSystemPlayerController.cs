...
public enum InputSystemStatus { idle, moving, crouching, sliding, climbingLadder, wallRunning, grabbedLedge, climbingLedge, vaulting, elevator }
...
public class InputSystemPlayerController : MonoBehaviour
{
    ...
    InputSystemPlayerMovement movement;
    InputSystemPlayerInput playerInput;
    ...
    {
        CreateVaultHelper();
        playerInput = GetComponentInChildren<InputSystemPlayerInput>();
        movement = GetComponent<InputSystemPlayerMovement>();

        if (GetComponentInChildren<AnimateLean>())
            animateLean = GetComponentInChildren<AnimateLean>();

        slideLimit = movement.IS_Controller.slopeLimit - .1f;
        radius = movement.IS_Controller.radius;
        height = movement.IS_Controller.height;
    ...
    void UpdateInteraction()
    {
        if (!canInteract)
        {
            if (movement.bIsGrounded || movement.myMoveDirection.y < 0)
                canInteract = true;
        }
    ...
    void DefaultMovement()
    {
        ...
        if (movement.bIsGrounded && playerInput.Jump())
        {
    ...
    void SlideMovement()
    {
        ...
        movement.Move(slideDir, movement.mySlideSpeed, 1f);
        if (slideTime <= 0)
        {
    ...
    void CheckSliding()
    {
        ...
        {
            slideDir = transform.forward;
            movement.IS_Controller.height = halfheight;
        ...
        if (Physics.Raycast(transform.position, -Vector3.up, out var hit, rayDistance))
        {
            ...
            if (angle > slideLimit && movement.myMoveDirection.y < 0)
            {
    ...
    bool canSlide()
    {
        if (!movement.bIsGrounded) return false;
    ...
    void CheckCrouching()
    {
        if (!movement.bIsGrounded || (int)status > 2) return;
    ...
    void Uncrouch()
    {
        movement.IS_Controller.height = height;
        status = Status.moving;
    }
    /*********************************************************************/
    ...
    void LadderMovement()
    {
        ...
        move *= input.x;
        move.y = input.y * movement.myWalkSpeed;

        bool goToGround = false;
        goToGround = (move.y < -0.02f && movement.bIsGrounded);
        ...
        if (!hasObjectInfront(0.05f, ladderLayer) || goToGround)
        {
            ...
            movement.ForceMove(pushUp, movement.myWalkSpeed, 0.25f, true);
        }
        else
    ...
    void WallrunningMovement()
    {
        ...
        if (!hasWallToSide(wallDir) || movement.bIsGrounded)
            status = Status.moving;

        movement.Move(move, movement.myRunSpeed, (1f - s) + (s / 4f));
    }

    void CheckForWallrun()
    {
        if (!canInteract || movement.bIsGrounded || movement.myMoveDirection.y >= 0)
            return;
    ...
    void ClimbLedgeMovement()
    {
        ...
        movement.Move(move, movement.myWalkSpeed, 0f);
    ...
    void UpdateLedgeGrabbing()
    {
        ...
        {
            if (canGrabLedge && !movement.bIsGrounded)
            {
                if (movement.myMoveDirection.y < 0)
                    CheckLedgeGrab();
            }
    ...
    void VaultMovement()
    {
        ...
        movement.Move(move, movement.myRunSpeed, 0f);
    }
    ...
    void CheckForVault()
    {
        ...
        checkDis += (movement.IS_Controller.velocity.magnitude / 16f); //Check farther if moving faster
        if (hasObjectInfront(checkDis, vaultLayer) && playerInput.Jump())
        {
            if (Physics.SphereCast(transform.position + (transform.forward * (radius - 0.25f)), 0.25f, transform.forward, out var sphereHit, checkDis, vaultLayer))
            {
                if (Physics.SphereCast(sphereHit.point + (Vector3.up * halfheight), radius, Vector3.down, out var hit, halfheight - radius, vaultLayer))
                {
                    ...
                    movement.IS_Controller.height = radius;
                }
            }
        }
    }
